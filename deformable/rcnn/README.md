...
Deformable ConvNets（github.com/msracver/Deformable-ConvNets）的 **rcnn** 在 Hwacha 上的一次前向，与 PyTorch 比对。
...
| 文件 | 内容 |
|---|---|
| `rcnn_tv.s` | hwacha-cc 生成的汇编，入口 `net` |
| `rcnn_tv_weights.bin.S` | 权重的 `.incbin` 桩（`split_weights.py` 分成 `.weights_lo` / `.weights_hi` 两段） |
| `rcnn_tv_weights.bin` | 权重 blob（不入 git，`make gen-rcnn` 按固定种子重建） |
| `rcnn_tv_check.bin` | 输入与 PyTorch 参考输出，host 用 `.incbin` 内嵌 |
...
```
make rcnn          # 编译 -> rcnn/rcnn_tv.riscv
make rcnn.spike    # 在 Spike 上运行
make gen-rcnn      # 从 PyTorch 重新生成
```
