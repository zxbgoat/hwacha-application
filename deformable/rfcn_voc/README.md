...
Deformable ConvNets（github.com/msracver/Deformable-ConvNets）的 **rfcn_voc** 在 Hwacha 上的一次前向，与 PyTorch 比对。
...
| 文件 | 内容 |
|---|---|
| `rfcn_voc_tv.s` | hwacha-cc 生成的汇编，入口 `net` |
| `rfcn_voc_tv_weights.bin.S` | 权重的 `.incbin` 桩（`split_weights.py` 分成 `.weights_lo` / `.weights_hi` 两段） |
| `rfcn_voc_tv_weights.bin` | 权重 blob（不入 git，`make gen-rfcn_voc` 按固定种子重建） |
| `rfcn_voc_tv_check.bin` | 输入与 PyTorch 参考输出，host 用 `.incbin` 内嵌 |
...
```
make rfcn_voc          # 编译 -> rfcn_voc/rfcn_voc_tv.riscv
make rfcn_voc.spike    # 在 Spike 上运行
make gen-rfcn_voc      # 从 PyTorch 重新生成
```
