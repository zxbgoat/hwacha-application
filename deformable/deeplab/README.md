...
Deformable ConvNets（github.com/msracver/Deformable-ConvNets）的 **deeplab** 在 Hwacha 上的一次前向，与 PyTorch 比对。
...
| 文件 | 内容 |
|---|---|
| `deeplab_tv.s` | hwacha-cc 生成的汇编，入口 `net` |
| `deeplab_tv_weights.bin.S` | 权重的 `.incbin` 桩（`split_weights.py` 分成 `.weights_lo` / `.weights_hi` 两段） |
| `deeplab_tv_weights.bin` | 权重 blob（不入 git，`make gen-deeplab` 按固定种子重建） |
| `deeplab_tv_check.bin` | 输入与 PyTorch 参考输出，host 用 `.incbin` 内嵌 |
...
```
make deeplab          # 编译 -> deeplab/deeplab_tv.riscv
make deeplab.spike    # 在 Spike 上运行
make gen-deeplab      # 从 PyTorch 重新生成
```
...
|---|---|
| 结果 | PASS |
| 周期数（rdcycle） | 80,933,539 |
| max\|diff\| | 0.002346 |
| max\|ref\| | 56.4798 |
| argmax（硬件 / 参考） | 61480 / 61480 |
