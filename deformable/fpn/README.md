...
Deformable ConvNets（github.com/msracver/Deformable-ConvNets）的 **fpn** 在 Hwacha 上的一次前向，与 PyTorch 比对。
...
| 文件 | 内容 |
|---|---|
| `fpn_tv.s` | hwacha-cc 生成的汇编，入口 `net` |
| `fpn_tv_weights.bin.S` | 权重的 `.incbin` 桩（`split_weights.py` 分成 `.weights_lo` / `.weights_hi` 两段） |
| `fpn_tv_weights.bin` | 权重 blob（不入 git，`make gen-fpn` 按固定种子重建） |
| `fpn_tv_check.bin` | 输入与 PyTorch 参考输出，host 用 `.incbin` 内嵌 |
...
```
make fpn          # 编译 -> fpn/fpn_tv.riscv
make fpn.spike    # 在 Spike 上运行
make gen-fpn      # 从 PyTorch 重新生成
```
...
|---|---|
| 结果 | PASS |
| 周期数（rdcycle） | 82,988,111 |
| max\|diff\| | 0.004383 |
| max\|ref\| | 99.3118 |
| argmax（硬件 / 参考） | 4530 / 4530 |
