// Shared by every Rodinia host: bare-metal helpers, the hwacha-cc launch conventions.
#include <stdio.h>
#include <string.h>
#include <math.h>
#include "util.h"
...
static int check_f(const char *tag, const float *hw, const float *ref, long n, float rtol) {
  ...
  printf("%s: %ld values, max|diff|=%ld/1e6 max|ref|=%ld/1e6, %d mismatches%s\n", tag, n, (long)(md * 1e6f), (long)(mx * 1e6f), bad, (hwacha_vl_short && hwacha_last_group) ? " (VL SHORT: group size not honoured)" : "");
  return bad;
}
