...
// does); the reference is the textbook 5/3 lifting with symmetric extension, low band in the first
// half of every row / column, high band in the second (the kernel's band writer layout)
#include "common.h"
#define SX 64
#define SY 64
#define WIN_SX 32
...
static void lift1d(const int *x, int n, int stride, int *lo, int *hi) {   // 5/3 forward lifting on one line
  ...
  for (int k = 0; k < nh; k++) h[k] = x[(2*k+1)*stride] - (int)floorf((x[mir(2*k, n)*stride] + x[mir(2*k+2, n)*stride]) / 2.0f);
  for (int k = 0; k < nl; k++) { int hl = h[k > 0 ? k - 1 : 0], hr = h[k < nh ? k : nh - 1]; if (k == 0) hl = h[0]; if (2*k+1 >= n) hr = h[k-1]; lo[k] = x[2*k*stride] + (int)floorf((hl + hr + 2) / 4.0f); }
  for (int k = 0; k < nh; k++) hi[k] = h[k]; }
int main(void) {
  for (int i = 0; i < SX*SY; i++) img[i] = rnd() % 256;
  unsigned long c0 = cyc();
  { static int lo[SX], hi[SX]; for (int y = 0; y < SY; y++) { lift1d(img + y*SX, SX, 1, lo, hi); for (int k = 0; k < SX/2; k++) { tmp[y*SX + k] = lo[k]; tmp[y*SX + SX/2 + k] = hi[k]; } }
    for (int x = 0; x < SX; x++) { lift1d(tmp + x, SY, SX, lo, hi); for (int k = 0; k < SY/2; k++) { ref[k*SX + x] = lo[k]; ref[(SY/2 + k)*SX + x] = hi[k]; } } }
