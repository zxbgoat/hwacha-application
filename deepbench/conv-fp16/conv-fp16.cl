/* DeepBench convolution in half precision (conv_bench with the "half" precision: cudnnConvolution*
 * with CUDNN_DATA_HALF tensors and float compute -- "FP16 inputs / FP32 math"). Same three passes
 * and the same direct form as conv.cl; the halves are converted on load (vlxh + vfcvt.s.h), the
 * float accumulator is rounded once on store (vfcvt.h.s). Written for hwacha-cc.
		...
		for (c = 0; c < C; c++)
			for (fy = 0; fy < R; fy++)
			{
				...
				for (fx = 0; fx < S; fx++)
				{
					...
					acc += (float)x[((n*C + c)*H + iy)*W + ix] * (float)f[((k*C + c)*R + fy)*S + fx];
				}
			}
		y[((n*K + k)*OH + oy)*OW + ox] = (half)acc;
	}
}
		...
		for (k = 0; k < K; k++)
			for (fy = 0; fy < R; fy++)
			{
				...
				for (fx = 0; fx < S; fx++)
				{
					...
					int ox = tx / ws;
					if (ox >= OW) continue;
					acc += (float)dy[((n*K + k)*OH + oy)*OW + ox] * (float)f[((k*C + c)*R + fy)*S + fx];
				}
			}
		dx[((n*C + c)*H + iy)*W + ix] = (half)acc;
	}
}
		...
		for (n = 0; n < N; n++)
			for (oy = 0; oy < OH; oy++)
			{
				...
				for (ox = 0; ox < OW; ox++)
				{
					int ix = ox*ws - pad_w + fx;
					if (ix < 0 || ix >= W) continue;
					acc += (float)dy[((n*K + k)*OH + oy)*OW + ox] * (float)x[((n*C + c)*H + iy)*W + ix];
				}
			}
		df[((k*C + c)*R + fy)*S + fx] = (half)acc;
	}
}
