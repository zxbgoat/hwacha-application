...
| gemm | level1 GEMM | `sgemmNN`, `sgemmNT` | 128 x 128 | PASS（`-vregs 32`，见下） |
...
**gemm 与 64 lane 的 work-group**：SHOC 的 sgemm 内核（源自 MAGMA）把 16 x 4 的 work-group 映射到 64 x 16 的 C
分块，B 分块经 `__local` 内存在 barrier 之间共享，所以 work-group 必须是 64 个 lane；Hwacha 的向量长度是
`maxvl = 8 * 256 / 向量寄存器数`，64 个 lane 要求内核不超过 32 个向量寄存器（`HWCCFLAGS_gemm := -vregs 32`）。
k 循环里 16 个 C 累加器、4 个 A 值和分块地址长期活跃，hwacha-cc 原来要 65 / 68 个寄存器；补了四处之后
（累加链原地写 phi 寄存器、共享 A 加载的 gather 偏移、uniform 出口的 LCSSA 值直接别名而不拷贝、phi 初值
不再活到循环末尾）降到 45 / 38，再加上溢出器就能放进 32 个，两个内核都精确通过。
...
在擦除的调用处插入指令、由流寄存器承载指针的 phi 不再申请寄存器）和上述四处寄存器压力改进。sort 要用 `-Xclang -disable-llvm-passes`
