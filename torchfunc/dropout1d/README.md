...
`torch.nn.functional.dropout1d`（dropout）的单函数测试，一次调用，与 PyTorch 逐元素比对。文档：https://docs.pytorch.org/docs/2.14/generated/torch.nn.functional.dropout1d.html

导出图计算：`F.dropout1d(x, 0.5, training=False)　（eval 模式：恒等）`
...
make gen-dropout1d      # 从 PyTorch 重新生成
```
