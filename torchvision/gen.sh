#!/bin/bash
# Regenerate one torchvision case from PyTorch: gen.sh <model> [HW] [kwargs-json] [hwacha-mlir flags]
...
M=$1; HW=${2:-32}; KW=${3:-{\}}; FLAGS=${4:-}
HERE=$(cd "$(dirname "$0")" && pwd)
W=$HERE/.build/$M; rm -rf "$W"; mkdir -p "$W" "$HERE/$M"
cd "$W"
T() { /usr/bin/time -f "[$1 maxrss=%MKB %es]" "${@:2}"; }
T export $PYTHON "$HERE/export_tv.py" $M ${M}_tv.mlir ${M}_tv_check.bin $HW "$KW"
